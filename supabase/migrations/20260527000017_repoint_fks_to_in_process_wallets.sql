...
SELECT DISTINCT addr FROM (
  ...
  UNION SELECT recipient      FROM public.in_process_transfers          WHERE recipient      IS NOT NULL
) sources
...
ALTER TABLE public.account_notifications
  ...
  ADD CONSTRAINT account_notifications_artist_address_fkey
...
ALTER TABLE public.in_process_admins
  ...
  ADD CONSTRAINT in_process_admins_artist_address_fkey
    FOREIGN KEY (artist_address) REFERENCES public.in_process_wallets(address)
    ON UPDATE CASCADE ON DELETE CASCADE;
...
ALTER TABLE public.in_process_arweave_uploads
  ...
  ADD CONSTRAINT in_process_arweave_uploads_artist_address_fkey
    FOREIGN KEY (artist_address) REFERENCES public.in_process_wallets(address)
    ON UPDATE CASCADE ON DELETE CASCADE;
...
ALTER TABLE public.in_process_moment_fee_recipients
  ...
  ADD CONSTRAINT in_process_moment_fee_recipients_artist_address_fkey
    FOREIGN KEY (artist_address) REFERENCES public.in_process_wallets(address)
    ON UPDATE CASCADE ON DELETE RESTRICT;

ALTER TABLE public.in_process_notifications
  DROP CONSTRAINT IF EXISTS in_process_notifications_artist_fkey,
  ADD CONSTRAINT in_process_notifications_artist_fkey
    FOREIGN KEY (artist) REFERENCES public.in_process_wallets(address)
    ON DELETE CASCADE;
