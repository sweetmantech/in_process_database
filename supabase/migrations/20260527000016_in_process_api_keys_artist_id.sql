...
ADD COLUMN IF NOT EXISTS artist_id UUID REFERENCES in_process_artists (id) ON DELETE CASCADE;
...
SET
  artist_id = w.artist
FROM
  in_process_wallets w
WHERE
  w.address = k.artist_address
...
WHERE
  ...
  AND id NOT IN (
    SELECT DISTINCT
      ON (k2.artist_id) k2.id
    FROM
      in_process_api_keys k2
      LEFT JOIN in_process_wallets w ON w.address = k2.artist_address
      AND w.type = 'external'
    WHERE
      k2.artist_id IS NOT NULL
    ORDER BY
      k2.artist_id,
...
CREATE INDEX if NOT EXISTS idx_in_process_api_keys_artist_id ON in_process_api_keys (artist_id);
