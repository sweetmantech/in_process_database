...
SET
  smart_wallet = LOWER(smart_wallet)
WHERE
  smart_wallet IS NOT NULL;
...
CREATE OR REPLACE FUNCTION public.in_process_artists_smart_wallet_lowercase () returns trigger language plpgsql AS $function$
BEGIN
...
BEFORE INSERT OR UPDATE OF smart_wallet ON public.in_process_artists FOR EACH ROW
