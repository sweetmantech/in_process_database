-- Fix race condition in upsert_artist_names: when two concurrent calls both
-- find no existing artist and both attempt INSERT, the second fails with
-- unique constraint on username. Use ON CONFLICT to handle this gracefully
-- by returning the existing artist's id instead of erroring.
