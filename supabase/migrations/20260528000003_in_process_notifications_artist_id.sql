...
WHERE w.address = n.artist AND w.artist IS NOT NULL;

-- ---------------------------------------------------------------------------
-- 3. Remove rows with no matching artist (orphaned wallet addresses)
-- ---------------------------------------------------------------------------
DELETE FROM public.in_process_notifications WHERE artist_id IS NULL;
...
-- ---------------------------------------------------------------------------
ALTER TABLE public.in_process_notifications
  ADD CONSTRAINT in_process_notifications_artist_id_fkey
