-- Add optional p_protocol filter to all three timeline RPCs.
-- Filter: (p_protocol IS NULL OR c.protocol::text = p_protocol)
-- DROP old overloads first so CREATE OR REPLACE does not leave dual signatures.
  ...
  IF p_curated AND p_mime IS NOT NULL THEN
    ...
    filtered_ids AS MATERIALIZED (
      ...
      INNER JOIN in_process_collections c ON c.creator = ca.address
        ...
        AND (p_protocol IS NULL OR c.protocol::text = p_protocol)
