-- Restore external_url filter in p_mime IS NULL branches of all 3 timeline RPCs.
-- The previous migration (20260617000004) removed both content.uri and external_url
-- filters from no-mime branches. The content.uri removal was intentional (pending
-- metadata should still show), but external_url must still exclude collector mints
-- (/collect/base:...) which are never original content.
-- Uses NOT EXISTS instead of a JOIN since metadata may not exist yet.
-- ── get_artist_timeline ───────────────────────────────────────────────────────
